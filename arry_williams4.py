...
class sim_stat :
    ...
    def update_stat(self, profit) :
        ...
        if self.max_loss > profit:
            self.max_loss = profit
        if self.max_gain < profit :
            self.max_gain = profit
...
def draw_chart_plotly(candle_data, ticker, stat) :
    ...
    for i in range(len(candle_data)-1, -1, -1)  :# 
        if 'profit' not in candle_data[i] :
            continue
        text = 'L'
        ...
        if candle_data[i]['profit'] != 0 :
            ...
            annotations.append(go.layout.Annotation(x=candle_data[i]['candleDateTimeKst'],
                                                    y=candle_data[i]['highPrice'],
                                                    xshift=1,  # x 축 기준으로 오른쪽으로 x칸 이동
                                                    yshift=25,  # y 축 기준으로 오른쪽으로 y칸 이동
                                                    showarrow=False,
                                                    text=candle_data[i]['trend']))
#            annotations.append(go.layout.Annotation(x=candle_data[i]['candleDateTimeKst'],
#                                                    y=candle_data[i]['highPrice'],
#                                                    xshift=1,  # x 축 기준으로 오른쪽으로 x칸 이동
#                                                    yshift=40,  # y 축 기준으로 오른쪽으로 y칸 이동
#                                                    showarrow=False,
#                                                    text=candle_data[i]['ma7']))

    # draw할 layout 생성
    width = len(candle_data) * 15
    layout = dict(
...
def simulation(ticker, sim_file, tr_login) :
    ...
    # 어제 일봉 setting
    tr_logic.update_new_candle(candle)

    deposit = 1000000   # 1회 최대 1백만원 매수
    balance = deposit   # 현재 잔고

    bought = 0          # 매수 중인지 여부
    trading_fee = 0.0005  # 매매 수수료
    ...
    for i in range(len(candle_data)-2,-1,-1)  :# 내림차순으로 저장되어 있으므로 뒤에서 한개씩
        ...
        candle = Candle()
        candle.new_candle(data)

        # simulation 중 파일에 저장할 변수들 추가
        save_mid_values(data)
        ...
        save_mid_values_jipyo(data, trend, ma7)

        if bought : # exit 조건 확인
            sell_price, losscut = tr_logic.is_exit_condition(candle)
            if sell_price > 0 : # exit 조건
                # 2. 일봉 종가에 판다고 가정
                profit = (sell_price - buy_price) * num_buying
                balance += profit
                ...
                # 5. adding log
                save_mid_values_buy_sell(data, losscut, sell_price) # tick과 같이 buy/sell 가격이 틀린 경우에는 buy_price에 1을 넣는다. 차후 excel에서 검증 편함
                save_mid_values_result(data, profit, stat.total_profit, dd)
            ...
            if buy_price > 0 : # 매수조건임
                ...
                # 5. adding log
                save_mid_values_buy_sell(data, buy_price, 0)
